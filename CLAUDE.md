...
5. `render_html_document` — assembles the standalone screen HTML for the optional `--html` output: same header/body/CSS as the PDF, wrapped in a `.sheet` div with a theme switch appended. Only called when `write_html=True`.
6. `build_resume` — assembles the full HTML document and calls WeasyPrint's `HTML(...).write_pdf(..., pdf_variant="pdf/ua-1")`. The output is always a tagged PDF/UA-1 (there is no opt-out). With `write_html=True` it then writes `out_path.with_suffix(".html")` and reports it via `BuildResult.html_path`; the HTML is written *after* the PDF so a WeasyPrint failure doesn't leave a lone `.html` implying success.

The `--html` output (CLI `--html`, default off) shares one stylesheet with the PDF. All screen presentation lives in a single `@media screen` block at the bottom of `style.css`; WeasyPrint renders with media type `print`, so that block never reaches the PDF. This is also why `position` is safe there despite the bullet-marker warning below — none of it touches the PDF content stream, and `.sheet` / `.theme-switch` don't exist in the PDF document at all. The theme switch (Auto/Light/Dark) is inline JS in `builder.py`: `system` is the *absence* of `data-theme` on `<html>` so CSS falls through to `prefers-color-scheme`, with `:root[data-theme="light"|"dark"]` spelled out in full to beat that media query in both directions. A prelude script in `<head>` applies the stored choice before first paint to avoid a flash. The build date is a real `.sheet-date` element in the HTML, since the PDF's `@page { @bottom-right }` margin box only applies when printing; `@media print` hides it so the two never double up.
...
`weasyprint` is imported lazily inside `build_resume`, not at module top level — this keeps `split_frontmatter`/`render_body_html`/`render_header_html`/`build_css` importable and unit-testable on systems without WeasyPrint's native libs (pango/harfbuzz/glib), e.g. CI. `tests/test_builder.py` only exercises these pure-logic functions for this reason; it never invokes WeasyPrint itself. The one exception is `tests/test_pdf_extraction.py`, which builds a real PDF and asserts on pypdf-extracted text (bullets inline and in document order — the content-stream contract ATS parsers and copy/paste rely on); it self-skips when WeasyPrint's native libs can't load.

Bullet markers in `style.css` must stay in normal inline flow. They were once absolutely positioned (`li { position: relative }` + `li::before { position: absolute }`), which scrambled the PDF content stream — positioned elements are painted in a later pass, so stream-order extractors (Workday, pypdf, Preview copy/paste) read every list after the *following* section heading. Don't reintroduce `position` on list markup; `tests/test_pdf_extraction.py` is the regression guard.
