...
`--html` (off by default) writes a browser-readable version alongside the PDF, same basename:

```bash
uv run resume --html                    # -> resume.pdf + resume.html
uv run resume --out out.pdf --html      # -> out.pdf + out.html
```

- **Self-contained.** One file, stylesheet inlined — email it or drop it on a static host as-is.
- **Sheet of paper.** A centred card on a tinted backdrop, capped at a readable measure. Not an exact reproduction of the page geometry.
- **Responsive.** Below ~800px the card goes full-bleed and the type sizes up.
- **Auto / Light / Dark switch**, top-right. `Auto` follows `prefers-color-scheme`; an explicit choice persists in `localStorage`.

Screen presentation lives in one `@media screen` block at the bottom of `style.css`. WeasyPrint renders with media type `print` and drops it, so one stylesheet serves both outputs and screen tweaks can't regress the PDF. The theme switch is also hidden under `@media print`.

Two differences from the PDF: the HTML is one continuous flow, so `<!-- pagebreak -->` markers do nothing and page count isn't reflected; and the build date is a real element rather than a `@page` margin box, which browsers apply only when printing. `--no-date` omits it from both.
...
style:
  ...
  # --html output only; ignored by the PDF
  "--sheet-width": "600pt"     # max width of the on-screen sheet
  "--sheet-padding": "44pt 48pt 48pt"
  "--screen-base-size": "12pt" # body type below ~800px
```
